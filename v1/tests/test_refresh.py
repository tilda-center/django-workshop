...
class RefreshTestCase(APITestCase):
    ...
    def test_success_refresh(self):
        ...
        access_token_data = jwt.decode(response.data['access'], options={'verify_signature': False})
        refresh_token_data = jwt.decode(response.data['refresh'], options={'verify_signature': False})
